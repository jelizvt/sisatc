...
@ManagedBean
@ViewScoped
public class ReporteDJVehicularManaged extends BaseManaged {
	...
	public ReporteDJVehicularManaged() {
	...
	public void imprimirPdf() {
		...
		try {
			...
			JasperPrint jasperPrint = JasperFillManager
					.fillReport((SATWEBParameterFactory.getPathReporte() + "reporte_dj_vehicular_pendientes.jasper"),
							parameters,
							connection);
			...
			response.addHeader("Content-Disposition", "attachment;filename=reporte_dj_vehicular_pendientes.pdf");
			...
			try {
			 ...
			 addErrorMessage(getMsg("Verifique las fechas."));
		}
	}

	public void imprimirXlsVehicular() {
		java.sql.Connection connection=null;
		try {
			CrudServiceBean connj=CrudServiceBean.getInstance();
			connection=connj.connectJasper();
			...
			JasperPrint jasperPrint = JasperFillManager.fillReport((SATWEBParameterFactory.getPathReporte()+"reporte_dj_vehicular_pendientes.jasper"),parameters,connection);
			ByteArrayOutputStream output = new ByteArrayOutputStream();
			...
			JRXlsExporter exporterXls = new JRXlsExporter();
			    ...
			    exporterXls.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, System.currentTimeMillis() + "Dj_Vehicular_Pendientes" + ".xls");  
			    exporterXls.setParameter(JRXlsExporterParameter.OUTPUT_STREAM, output);  
			    exporterXls.exportReport();

			HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
			response.setContentType("application/vnd.ms-excel");  
		    response.setHeader("Content-Disposition", "attachment;filename=" + System.currentTimeMillis() + "Dj_Vehicular_Pendientes.xls");  
			response.setContentLength(output.size());
			ServletOutputStream servletOutputStream = response.getOutputStream();
			servletOutputStream.write(output.toByteArray(), 0, output.size());
			servletOutputStream.flush();
	...
	public Boolean validarFechas(){	
		...
		if (fechaInicio.equals(fechaFin)|| fechaInicio.before(fechaFin)) {
			fechaCorrecta = true;
		} else {
			fechaCorrecta = false;
		}
...
//		if (validarFechas()==true){
		...
		try {
			...
			JasperPrint jasperPrint = JasperFillManager
					.fillReport((SATWEBParameterFactory.getPathReporte() + "reportedjvehicularconsolidado.jasper"),
							parameters,
							connection);
			...
			HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext()
					.getResponse();
			response.setContentType("application/pdf");
			response.addHeader("Content-Disposition", "attachment;filename=Dj_Vehicular_Consolidado.pdf");
...
//	 }
	...
	public void imprimirConsolidadoXLS() {
		...
		try {
			...
			JasperPrint jasperPrint = JasperFillManager.fillReport((SATWEBParameterFactory.getPathReporte()+"reportedjvehicularconsolidado.jasper"),parameters,connection);
			ByteArrayOutputStream output = new ByteArrayOutputStream();
			...
			JRXlsExporter exporterXls = new JRXlsExporter();
			    ...
			    exporterXls.setParameter(JRXlsExporterParameter.MAXIMUM_ROWS_PER_SHEET, 20000); 
			    exporterXls.setParameter(JRXlsExporterParameter.IS_DETECT_CELL_TYPE, true);  
			    exporterXls.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, System.currentTimeMillis() + "Dj_Vehicular_Consolidado" + ".xls");  
			    exporterXls.setParameter(JRXlsExporterParameter.OUTPUT_STREAM, output);  
			    exporterXls.exportReport();

			HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();
			response.setContentType("application/vnd.ms-excel");  
		    response.setHeader("Content-Disposition", "attachment;filename=" + System.currentTimeMillis() + "Dj_Vehicular_Consolidado.xls");  
			response.setContentLength(output.size());
			ServletOutputStream servletOutputStream = response.getOutputStream();
			servletOutputStream.write(output.toByteArray(), 0, output.size());
			servletOutputStream.flush();
