...
import com.sat.sisat.coactiva.entity.CoactivaMultasTransito2013;
...
@ManagedBean
@ViewScoped
public class MultasTransito2013Managed extends BaseManaged {
	@EJB
	...
	public void buscar(){
		try{
			...
			if((papeleta!=null&&papeleta.trim().length()>0)||(nroValor!=null&&nroValor.trim().length()>0)||(placa!=null&&placa.trim().length()>0)){
				lista=controlycobranzaBo.getAllMultasTransito2013(papeleta,nroValor,placa);
			}
		}catch(Exception e){
			e.printStackTrace();
	...
	public void exportHtmlTableToExcel() throws IOException{       
		try{
			List<CobranzaCoactiva> listaAll=controlycobranzaBo.getAllMultasTransito2013();
...
//			htmlBuffer.append("<td>MONTO_CANCELADO (MAY-NOV 2013)</td>");
			...
			while (it2.hasNext()) {
				CoactivaMultasTransito2013 obj = (CoactivaMultasTransito2013)it2.next();
				htmlBuffer.append("<tr>");
...
//					htmlBuffer.append("<td>").append(obj.getMonto_pago1()==null?"":obj.getMonto_pago1()).append("</td>");
			...
			//DateTimeFormatter fmt = DateTimeFormat.forPattern("yyyy-MM-dd_HHmmss");
			  String filename = "multas_transito_2013_"+System.currentTimeMillis() + ".xls";
			  //Setup the output
