...
@ManagedBean
@ViewScoped
public class CalculoAlcabalaManaged extends BaseManaged {
	...
	public CalculoAlcabalaManaged(){
		getSessionManaged().setLinkRegresar("/sisat/alcabala/busquedaalcabala.xhtml");
	...
	private HtmlComboBox cmbCondicionPropiedad;
	private HtmlComboBox cmbTipoTransferencia;
	private HtmlComboBox cmbTipoNotaria;
	...
	private HashMap<String, Integer> mapRpTipoNotaria = new HashMap<String, Integer>();
	private List<SelectItem> lstNotaria = new ArrayList<SelectItem>();
	private List<SelectItem> lstTipoTransferencia = new ArrayList<SelectItem>();
	private List<SelectItem> lstTipoMoneda = new ArrayList<SelectItem>();
	private HashMap<String, Integer> mapTipoTransferencia = new HashMap<String, Integer>();
	...
	private String cmbTipoNotariaValor;
	private String cmbTipoTransferenciaValor;
	private String cmbCondiPropiedadValor;
	...
	public void setProperty(BuscarAlcabalaDTO currentItem){
		...
		//combo Ubicacion Propiedad
		if(currentItem.getUbicPredio().equals("D")){
			setTipoUbicacionPropiedad1("Dentro de la Jurisdiccion");
			setFueraDistrito(false);
			setDistrital(true);
			
		}else if(currentItem.getUbicPredio().equals("F")){
			setTipoUbicacionPropiedad1("Fuera de la Jurisdiccion");
			setFueraDistrito(true);
			setDistrital(false);
		}
		//notaria combo
		mapRpTipoNotaria.get(0);
		
		Iterator itNota = mapRpTipoNotaria.entrySet().iterator();
//		System.out.println(currentItem.getNotariaId());
		while (itNota.hasNext()) {
			Map.Entry e = (Map.Entry)itNota.next();
			if((Integer)e.getValue()==currentItem.getNotariaId()){
				setCmbTipoNotariaValor(String.valueOf(e.getKey()));
			}
		}
		
		tipoNotaria=currentItem.getNotariaId();
		
		//combotipoTransferencia
		Iterator itTipoTrans = mapTipoTransferencia.entrySet().iterator();
//		System.out.println(currentItem.getTipoTransferenciaId());
		
		if(currentItem.getTipoTransferenciaId()>15){
			setInafectoImpuesto(Boolean.TRUE);
		}
		while(itTipoTrans.hasNext()){
			Map.Entry e=(Map.Entry)itTipoTrans.next();
			if((Integer)e.getValue()==currentItem.getTipoTransferenciaId()){
				setCmbTipoTransferenciaValor(String.valueOf(e.getKey()));
			}

		}
		tipoTransferencia=currentItem.getTipoTransferenciaId();
		
		//combocondicionporpiedad
		Iterator itCondi = mapRpCondicionPropiedad.entrySet().iterator();
//		System.out.println(currentItem.getCondicionPropiedadId());
		while (itCondi.hasNext()) {
			Map.Entry e=(Map.Entry)itCondi.next();
			if((Integer)e.getValue()==currentItem.getCondicionPropiedadId()){
				setCmbCondiPropiedadValor(String.valueOf(e.getKey()));
			}
			
		}
		condicionPropiedadId=currentItem.getCondicionPropiedadId();
...
//		System.out.println(currentItem.getTipoMonedaId());
		...
		//transferente
		try {
			ArrayList<BuscarPersonaDTO> lstTransferentes= alcabalaBo.getRaTransferenteAlcabalaByDJAlcabajaId(currentItem.getDjAlcabalaId());
			setLstTransferentesAnterior(lstTransferentes);
			
		if(lstTransferentes.size()>0){
				
		AgregarTransferenteAlcabalaManaged agregarTransferenteAlcabala=(AgregarTransferenteAlcabalaManaged) getManaged("agregarTransferenteAlcabalaManaged");
		agregarTransferenteAlcabala.setProperty(lstTransferentes);
			...
			//docuanexos
		try {
			ArrayList<AnexosDeclaVehicDTO> raAlcaSus= alcabalaBo.getRaAlcabalaSustentoByDjAlcabalaId(currentItem.getDjAlcabalaId());

		if(raAlcaSus.size()>0){
			
		SustentoAlcabalaManaged sustenAlcabala=(SustentoAlcabalaManaged)getManaged("sustentoAlcabalaManaged");
		sustenAlcabala.setProperty(raAlcaSus);
		setLstAnexosMuestra(raAlcaSus);
		...
		if(currentItem.getFlagPrimeraVenta().equals("1")){
			setPrimeraVEnta(Boolean.TRUE);
		}else if(currentItem.getFlagPrimeraVenta().equals("2")){
	...
	public void mostrarFueraDistrito(ValueChangeEvent event) {

		try {
			getCmbUbicacion().toString();
			HtmlComboBox combo = (HtmlComboBox) event.getComponent();
			String value = combo.getValue().toString();
						
			if (value.equals("Fuera de la Jurisdiccion")) {
				setFueraDistrito(true);
				setDistrital(false);

			} else if (value.equals("Dentro de la Jurisdiccion")) {
				setFueraDistrito(false);
				setDistrital(true);
				
			} else if (!value.equals("Fuera de la Jurisdiccion")) {
				setFueraDistrito(false);
				setDistrital(true);
			}
	...
	public boolean isDocuIdenti() {
		return docuIdenti;
	}

	public void setDocuIdenti(boolean docuIdenti) {
		this.docuIdenti = docuIdenti;
	}
	...
	public HashMap<String, Integer> getMapRpTipoNotaria() {
		return mapRpTipoNotaria;
	}

	public void setMapRpTipoNotaria(HashMap<String, Integer> mapRpTipoNotaria) {
		this.mapRpTipoNotaria = mapRpTipoNotaria;
	}

	public Integer tipoNotaria;

	public void loadNotariaById(ValueChangeEvent event) {
	...
	public void init() {
		try {
			...
			// CARGANDO NOTARIA
			List<GnNotaria> lstSNotaria = alcabalaBo.getAllGnNotaria();
			Iterator<GnNotaria> it1 = lstSNotaria.iterator();
			while (it1.hasNext()) {
				GnNotaria obj = it1.next();
				lstNotaria.add(new SelectItem(obj.getNombreNotaria(), String.valueOf(obj.getNotariaId())));
				mapRpTipoNotaria.put(obj.getNombreNotaria(), obj.getNotariaId());
			}

			// CARGANDO TIPO TRANSFERENCIA
			List<RaTipoTransferencia> lstSTipoTransferencia = alcabalaBo.getAllRaTipoTransferencia();
			Iterator<RaTipoTransferencia> it2 = lstSTipoTransferencia.iterator();
			while (it2.hasNext()) {
				RaTipoTransferencia obj = it2.next();
				lstTipoTransferencia.add(new SelectItem(obj.getDescCorta(),	String.valueOf(obj.getTipoTransferenciaId())));
				mapTipoTransferencia.put(obj.getDescCorta(),obj.getTipoTransferenciaId());
	...
	public void loadTipoTransferencia(ValueChangeEvent event) {
		try {
			String value = (String) event.getNewValue();
			if (value.length()>0&&value!="--") {
				tipoTransferencia = (Integer) mapTipoTransferencia.get(value);
				
			}

			if(tipoTransferencia!=null && tipoTransferencia<16 && datosInafec==null ){
				setInafectoImpuesto(Boolean.FALSE);
			}
			
			if(tipoTransferencia!=null && tipoTransferencia>15){
				setInafectoImpuesto(Boolean.TRUE);
			}
	...
	try {
		...
		if((predioId>0)&&(getFechaTransferencia()!=null)){
...
//			String fechaVence = "" + DateUtil.obtenerAnioSegunFecha(fechaTransferencia) + "-" + mesVence + "-"+ diaVence + "";
			...
			/**
			 *  Calculando la fecha de vencimiento de la declaración
			 *  Es el ultimo dia del mes siguiente a la fecha de transferencia
			 */
			calen.setTime(fechaTransferencia);			
			
			calen.add(Calendar.MONTH, 1);										//moviendo al siguiente mes
			System.out.println(calen);
			calen.set(Calendar.DATE, calen.getActualMaximum(Calendar.DATE)); 			// moviendo al ultimo dia
			calen.set(Calendar.HOUR_OF_DAY, 23);
			calen.set(Calendar.MINUTE,59);
			calen.set(Calendar.SECOND, 59);
			
			fechaVenceD = calen.getTime();										// seteando la fecha
...
//			Date fechaVenceD = formatoFecha.parse(fechaVence);			
			...
			Date fechaVenceD = calen.getTime();
	...
	public void vistaPrevia()throws Exception{
		...
		impAlcaDTO.setNotaria(cmbTipoNotariaValor);
		impAlcaDTO.setTipoTransferencia(cmbTipoTransferenciaValor);
		impAlcaDTO.setCondicionPropiedad(cmbCondiPropiedadValor);
	...
	public void actualizar(){
		try {
			...
			//CAMBIA ESTADO A ANEXOS
			alcabalaBo.actualizaAlcabalaSustento(Integer.parseInt(djAlcabala));
			//GUARDA ANEXOS
			if(getLstAnexosMuestra().size()>0){
			guardarAnexos(getLstAnexosMuestra());
			}
			
			//CAMBIA ESTADO TRANSFERENTES
			alcabalaBo.actualizaRaTransferente(Integer.parseInt(djAlcabala));
			// GUARDANDO TRANSFERENTES
			if(getAgregarTransferenteAlcabalaManaged().getLstTransferentes().size()>0){
				guardarTransferentes(getAgregarTransferenteAlcabalaManaged().getLstTransferentes());				
			}
	...
	public RaDjalcabala obtenerDJAlcabala(){
		...
		if(tipoNotaria!=null){
		raDjalcabala.setNotariaId(tipoNotaria);
		}
	...
	public SustentoAlcabalaManaged getSustentoAlcabalaManaged() {
		return (SustentoAlcabalaManaged) getManaged("sustentoAlacabalaManaged");
	}
	...
	public List<SelectItem> getLstNotaria() {
		return lstNotaria;
	}

	public void setLstNotaria(List<SelectItem> lstNotaria) {
		this.lstNotaria = lstNotaria;
	}

	public List<SelectItem> getLstTipoTransferencia() {
		return lstTipoTransferencia;
	}

	public void setLstTransferencia(List<SelectItem> lstTipoTransferencia) {
		this.lstTipoTransferencia = lstTipoTransferencia;
	}
	...
	public HtmlComboBox getCmbCondicionPropiedad() {
		return cmbCondicionPropiedad;
	}

	public void setCmbCondicionPropiedad(HtmlComboBox cmbCondicionPropiedad) {
		this.cmbCondicionPropiedad = cmbCondicionPropiedad;
	}
	...
	public HtmlComboBox getCmbTipoTransferencia() {
		return cmbTipoTransferencia;
	}

	public void setCmbTipoTransferencia(HtmlComboBox cmbTipoTransferencia) {
		this.cmbTipoTransferencia = cmbTipoTransferencia;
	}

	public HtmlComboBox getCmbTipoNotaria() {
		return cmbTipoNotaria;
	}

	public void setCmbTipoNotaria(HtmlComboBox cmbTipoNotaria) {
		this.cmbTipoNotaria = cmbTipoNotaria;
	}
	...
	public String getCmbTipoNotariaValor() {
		return cmbTipoNotariaValor;
	}

	public void setCmbTipoNotariaValor(String cmbTipoNotariaValor) {
		this.cmbTipoNotariaValor = cmbTipoNotariaValor;
	}
	...
	public String getCmbCondiPropiedadValor() {
		return cmbCondiPropiedadValor;
	}

	public void setCmbCondiPropiedadValor(String cmbCondiPropiedadValor) {
		this.cmbCondiPropiedadValor = cmbCondiPropiedadValor;
	}
