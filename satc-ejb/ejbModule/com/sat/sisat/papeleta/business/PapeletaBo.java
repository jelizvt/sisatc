...
public class PapeletaBo extends BaseBusiness implements PapeletaBoRemote{
	 ...
	 * @throws Exception
	 */
	public Integer solo_guardarPaPersona(PaPersona paPersona)
	{
		...
		if(gnPersona==null){
			if(paPersona.getPersonaId()==Constante.RESULT_PENDING){
				...
				//Guardamos datos de auditoria				
				//getService().auditarPersona(paPersona, paPersonaOriginal,getUser().getTerminal(),getUser().getUsuarioId());
	...
	public Integer guardarPaPersona(PaPersona paPersona,BuscarPersonaDTO paPersonaOriginal,String terminal,Integer usuarioId)throws Exception{
		GnPersona gnPersona=getService().find(paPersona.getPersonaId(), GnPersona.class);
		
		if (gnPersona==null && paPersona.getPersonaId()==Constante.RESULT_PENDING) 
			{
			Integer Id=getService().ObtenerCorrelativoTabla("gn_persona", 1);
			gnPersona=getGnPersona(paPersona);
			gnPersona.setPersonaId(Id);
		...
		else
		{
			...
			getService().auditarPersona(paPersona, paPersonaOriginal,terminal,usuarioId);
		}
...
//			incidenciaAntigua.setIncidenciaId(incidenciaId);
	...
	public int guardarOperacionAuditoria (GnAuditoriaOperacion auditoria) throws Exception {
		return getService().guardarOperacionAuditoria(auditoria);
	}
