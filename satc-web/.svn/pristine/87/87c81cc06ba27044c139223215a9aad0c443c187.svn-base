...
@ManagedBean
@ViewScoped
public class CuentaCorrienteSimuladorManaged extends BaseManaged implements Serializable {
	...
	public CuentaCorrienteSimuladorManaged(){
...
//	}
	...
	public void initialize(){
		try {
			...
			if (cjPersona != null) {
				...
				listCtaCte = cajaBo.obtenerDeuda(cajeroId, personaId,"","","","",DateUtil.dateToSqlTimestamp(getFechaConsulta()));
				calcularTotales();
	...
	public String agregar() {
		...
		try {
			...
			deudas = idDeudasCarrito.toString().replace("[", "").replace("]", "");
...
//					}
				...
				String deudas = idDeudasCarrito.toString().replace("[", "").replace("]", "");
				//listDeudaCobrar = new ArrayList<DeudaDTO>();
				listDeudaCobrar = cajaBo.obetenerDeudaConDsctos(deudas, fechaConsulta, getSessionManaged().getUsuarioLogIn().getUsuarioId(),1);
				calcularTotalesCabrar();
			}
		} catch (Exception ex) {
	...
	public String cobrar(){
		...
		getCajaCobranzaManaged().setIdDeudasACobrar(idDeudasCarrito.toString().replace("[", "").replace("]", ""));
		return null;
	}
