...
@ManagedBean
@ViewScoped
public class ObligacionManaged extends BaseManaged {
	...
	private int MULTAS_MPC = 14;
	private int MULTAS_DRTC = 15;
...
//																true);
//		
//			lstDetencionDeudaDTOs = obligacionBo.buscarDetenciones(getSessionManaged().getContribuyente().getPersonaId());
...
//		} 		
	...
	public void changeListenerValueCmbConcepto(ValueChangeEvent e) {
		...
		if (getObligacionDTO().getAnnoAfectacion() <= 0) {
			...
			message.setSummary("Debe ingresar un aÃ±o de afectaciÃ³n");
			message.setDetail(" * Debe ingresar un aÃ±o de afectaciÃ³n");
			...
			try {
				...
				if(cmbConcepto.equals("MULTAS DRTPE")){
					getObligacionDTO().setConceptoId(this.MULTAS_DRTPE);
					getObligacionDTO().setConceptoDescripcion("MULTAS DRTPE");
					listSubConceptoDTOs = obligacionBo.getSubConceptoMULTASDRTPE(obligacionDTO.getAnnoAfectacion());
				}

				if(cmbConcepto.equals("MULTAS MPC")){
					getObligacionDTO().setConceptoId(this.MULTAS_MPC);
					getObligacionDTO().setConceptoDescripcion("MULTAS MPC");
					listSubConceptoDTOs = obligacionBo.getSubConceptoMULTA(this.MULTAS_MPC);
				}
				
				if(cmbConcepto.equals("MULTAS DRTC")){
					getObligacionDTO().setConceptoId(this.MULTAS_DRTC);
					getObligacionDTO().setConceptoDescripcion("MULTAS DRTC");
					listSubConceptoDTOs = obligacionBo.getSubConceptoMULTA(this.MULTAS_DRTC);
				}
				...
				if (listSubConceptoDTOs.size() == 0 && !cmbConcepto.isEmpty()) {
					WebMessages.messageInfo("No se encontraron subconceptos asociados al aÃ±o de afectaciÃ³n ingresado y al concepto elegido");
					cmbConcepto = null;
	...
	/**
	 * MÃ©todo que limpia el campo <code>codidoPlacaReferenciaObg</code> para quitar la eleccion de un predio o un vehiculo
	 */
	...
	public List<SelectItem> getListItemConcepto() {
		...
		listSelectItems.add(new SelectItem("MULTAS DRTPE", String.valueOf(this.MULTAS_DRTPE)));
		listSelectItems.add(new SelectItem("MULTAS MPC", String.valueOf(this.MULTAS_MPC)));
		listSelectItems.add(new SelectItem("MULTAS DRTC", String.valueOf(this.MULTAS_DRTC)));
//		listSelectItems.add(new SelectItem("MULTAS", String.valueOf(this.MULTAS)));		
	...
	/**
	 * MÃ©todo que detecta el cambio en el campo base imponible, para luego actualizar el valor que aparecera en el campo
	 * impuesto.
	 * @param event
	 */
	...
	/**
	 * MÃ©todo que realiza el registro que todos los items de la grilla, asi como el registro de las deudas generadas.
	 */
	...
	public void desactivarDetencion(Integer personaId, Integer conceptoId, Integer subConceptoId, Integer anho, Integer determinacionId) {

//		try {
//			obligacionBo.desactivarDetencion(personaId, conceptoId, subConceptoId, anho, determinacionId);
//			
//			lstDetencionDeudaDTOs = obligacionBo.buscarDetenciones(getSessionManaged().getContribuyente().getPersonaId());
//			
//		} catch (SisatException e) {
...
//		}
	...
	public void activarDetencion(Integer personaId, Integer conceptoId, Integer subConceptoId, Integer anho, Integer determinacionId) {

//		try {
//			obligacionBo.activarDetencion(personaId, conceptoId, subConceptoId, anho, determinacionId);
//			
//			lstDetencionDeudaDTOs = obligacionBo.buscarDetenciones(getSessionManaged().getContribuyente().getPersonaId());
//			
//		} catch (SisatException e) {
