...
import com.sat.sisat.caja.managed.CajaPersonaManaged;
...
@ManagedBean
@ViewScoped
public class CajaCobranzaPapeletaSimuladorManaged extends BaseManaged implements Serializable {
	...
	public CajaCobranzaPapeletaSimuladorManaged() {
	}
	...
	public void iniciarDatosDefault() {
		try {
			...
			if (deudasPapeletasSeleccionadas != null) {
				...
				this.listPapeletas = cajaBo.obtenerPapeletaResumenPorDeudasId(idDeudasACobrar, 1);
				
				this.listResumenObligacionDTOs = cajaBo.obtenerResumenObligacionesPorDeudasId(idDeudasACobrar, 1);
				...
				if (listPapeletas != null && listPapeletas.size() > 0) {
					...
					List<DeudaDTO> listDeudaCobrar = cajaBo.obetenerDeudaConDsctos(idDeudasACobrar, Calendar
							.getInstance().getTime(), getSessionManaged().getUsuarioLogIn().getUsuarioId(), 1);
