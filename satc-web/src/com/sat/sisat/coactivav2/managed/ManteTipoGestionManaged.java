...
@ManagedBean
@ViewScoped
public class ManteTipoGestionManaged extends BaseManaged {
	...
	public void init() {
		try {
			records=cobranzaCoactivaBo.consultarTipoGestion();
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void guardar(){
		try{
			if(selected!=null){
				cobranzaCoactivaBo.registraTipoGestion(selected.getId(), selected.getDescripcion(), selected.getEstado()?1:9, getSessionManaged().getUsuarioLogIn().getUsuarioId(),getSessionManaged().getTerminalLogIn());
				records=cobranzaCoactivaBo.consultarTipoGestion();
			}
		}catch(Exception e){
			e.printStackTrace();
