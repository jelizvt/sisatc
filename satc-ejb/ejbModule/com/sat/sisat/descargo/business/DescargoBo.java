...
//      
    ...
    */
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<GnTipoDocumento> obtenerTipoDocumentosDescargo(int usuarioId) throws SisatException {
		
		List<GnTipoDocumento> result  = null;
		try{
			
			
			int status = getService().getStatusFisca(usuarioId);
			
			if(status == 0) {
				Query q = em.createQuery("from GnTipoDocumento where flag_descargo =1"); 
				result = q.getResultList();
			}else {
				Query q = em.createQuery("from GnTipoDocumento where flag_descargo in (1,2)"); 
				result = q.getResultList();
			}
