...
@ManagedBean
@ViewScoped
public class EstadoCuentaDetalladoManaged  extends BaseManaged{
	/**
	 * 
	 */
	private static final long serialVersionUID = -6469854545302307657L;
	...
	public void imprimirEstadoCuenta()throws SQLException {
		...
		try {
			CrudServiceBean connj=CrudServiceBean.getInstance();
		     ...
		     JasperPrint jasperPrint = JasperFillManager.fillReport((SATWEBParameterFactory.getPathReporte()+"ec_estado_cuenta_detallado.jasper"), parameters,connection);
		     ByteArrayOutputStream output = new ByteArrayOutputStream();
		     JasperExportManager.exportReportToPdfStream(jasperPrint, output);	                           
		     HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext().getResponse();		         		          
		     response.setContentType("application/pdf");	         
		     response.addHeader("Content-Disposition","attachment;filename=ec_estado_cuenta_detallado"+personaId+".pdf");
