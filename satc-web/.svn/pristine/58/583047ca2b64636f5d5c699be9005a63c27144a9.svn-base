...
public class DeterminacionArbitriosMasiva {
	//static Logger log = Logger.getLogger(DeterminacionArbitriosMasiva.class);
	...
	HashMap<Integer,Integer> mapCategoriaUsoId2014=new HashMap<Integer,Integer>();
	HashMap<Integer,Integer> mapCategoriaSeguridadUsoId2014=new HashMap<Integer,Integer>();
	
	HashMap<String,DtZonaSeguridadUso2014> mapDtZonaSeguridadUso2014=new HashMap<String,DtZonaSeguridadUso2014>();
	
	public DeterminacionArbitriosMasiva(CalculoPredialBoRemote calculoPredialBo,GeneralBoRemote generalBo,Integer anio){
		try{
...
//	        	RpCategoriaUso obj = it0.next();  
			for(RpCategoriaUso obj  : lstRpCategoriaUso){
...
//			RpCategoriaUsoSeguridad obj = its0.next();
			for(RpCategoriaUsoSeguridad obj : lstRpCategoriaUsoSeguridad){
//	        	  
...
//	        	RpTipoUso obj = it5.next();  
			for(RpTipoUso obj : lstRpTipoUso){
...
//	        	DtZonaSeguridad obj = it6.next();  
			for(DtZonaSeguridad obj : lstDtZonaSeguridad){
...
//	        	DtMatrizRecojo obj = it3.next();  
	        for(DtMatrizRecojo obj: lrecojoUso ){
...
//	        	TipoUsoDTO obj = it4.next();  
	        for(TipoUsoDTO obj :lCategoriaUsoId2014){
...
//	        	TipoUsoDTO obj = it7.next();  
	        for(TipoUsoDTO obj : lcategoriaSeguridadTipoUso2014){
...
//	        	DtZonaSeguridadUso2014 obj = itm.next();  
	        for(DtZonaSeguridadUso2014 obj:lZonaSeguridadUso2014 ){
	 ...
	 */
	public void generarDeterminacionArbitrios2014(Integer personaId,Integer anio)throws Exception{
...
//				RpDjpredial rpDjPredial=lDjPredial.get(i);
			...
			for(RpDjpredial rpDjPredial:lDjPredial){				
				if(rpDjPredial.getTipoPredio().equals(Constante.TIPO_PREDIO_URBANO)){
					...
					if(determinacionAnt!=null){
						...
						//getCalculoPredialBo().inactivaDeterminacionAnterior(personaId, anio, Constante.CONCEPTO_ARBITRIO,Constante.ESTADO_ACTIVO,-1);
					}
					calculoArbitrios2014(rpDjPredial,anio,null);	
				}else{
...
//		}
	...
	private DeterminacionArbitriosDTO calculoArbitrios2014(RpDjpredial djpredial,Integer anio,List<DtDeterminacion> lDeterminacionAnt)throws Exception{
		...
		try{
			System.out.println("Determinacion arbitrios para persona_id: "+djpredial.getPersonaId());
